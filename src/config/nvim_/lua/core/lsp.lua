...
local function setup_mappings(client, bufnr)
  ...
  local mappings = {
    ...
    { 'n', ']c', prev_diagnostic(0), desc = 'go to prev diagnostic' },
    { 'n', '[c', next_diagnostic(0), desc = 'go to next diagnostic' },
    ...
    { 'n', 'gy', lsp.buf.type_definition, desc = 'go to type definition', capability = provider.DEFINITION },
...
local function max_diagnostic(callback)
  return function(_, bufnr, diagnostics, opts)
    local max_severity_per_line = mines.fold(function(diag_map, d)
...
function mines.lsp.get_clients(opts)
